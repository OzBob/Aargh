...
var previews = [{ index: -1 }, { index: 0 }, { index: 1 }];
var mainSozi = void 0;
...
function updateNotes() {
    var notes = previews[1].sozi.player.currentFrame.notes;
...
function load(url) {
    ...
    previews.forEach(function (p, i) {
        onSoziLoaded(iframes[i].contentWindow, function (sozi) {
            p.sozi = sozi;

            sozi.player.disableMedia();
            sozi.player.pause();

            sozi.presentation.enableMouseTranslation = sozi.presentation.enableMouseNavigation = sozi.presentation.enableKeyboardZoom = sozi.presentation.enableKeyboardRotation = sozi.presentation.enableKeyboardNavigation = false;
    ...
    var presWindow = window.open(url, "sozi-presentation", "width=600, height=400, scrollbars=yes");
    try {
        presWindow.focus();
        // When the presentation is fully loaded and initialized,
        // setup the "frameChange" handler.
        onSoziLoaded(presWindow, function (sozi) {
            mainSozi = sozi;
